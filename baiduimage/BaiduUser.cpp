...
QNetworkReply *_PrivateBaiduUserNetworkAccessManager::createRequest(
    ...
    ++$m$replyCount;
    qDebug()<<"+"<<$m$replyCount.load();
    ans->connect(ans,&QObject::destroyed,this,[this]() {
        --$m$replyCount;
        qDebug()<<"-"<<$m$replyCount.load();
    });
#endif
...
QByteArray BaiduUser::gid() {
    ...
    class Array {
        char data[35];
    public:
        Array():data{ toHex0[std::rand()&15],toHex0[std::rand()&15],toHex0[std::rand()&15],
...
inline void DownLoadBaiduImage::update_data() {
    ...
    for (const auto & varI:varItems) {
        auto item=memory::make_shared<BaiduImage::Item>();
        {
            ...
            auto varImageName=
                QByteArray::number(varI->image_index).rightJustified(16,'0');
            ...
            if (suffixPos!=objurl.npos) {
                ...
                if ((suffixSize<6)&&(suffixSize>1)) {
                    varImageName.append(objurl.data()+suffixPos,suffixSize);
                }
                else {
